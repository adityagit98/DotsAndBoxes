public class MinMax {
	...
	int[] makeMove(State node, int depth)
	{
		...
		coords = minMax(grida,changedminscore,changedmaxscore,type, depth, Integer.MIN_VALUE, Integer.MAX_VALUE);
		return coords;
			...
			int flag = 0;			//if(alpha<=beta) then for breaking from  inner loop we use flag value;
				for(int row = 0;flag == 0 && row < grid.length; row++)	//run for all possible moves 
				{
					if(row % 2 == 0)
					{
						...
						{
							if(grid[row][col] < 1)		//here place is empty we can make move
							{	
